...
#try to use modified ResNet generator
...
def create_dataset(data_dir):
    ...
    for folder, subfolders, files in os.walk(data_dir):
        ...
        for file in files:
            ...
            tensors.append(orig_img)
            x+=1
            
    dataset = tf.data.Dataset.from_tensor_slices(tensors)
    # dataset = tf.data.TFRecordDataset(for file in td.data.Dataset.list_files(file_pattern,shuffle=None,seed=None))
    return dataset
...
#cycle consistency loss (x->y->x dif between x's)
def calc_cycle_loss(real_image, cycled_image):
    loss1 = tf.reduce_mean(tf.abs(real_image - cycled_image))
    return LAMBDA * loss1

#identity loss (g(x) should = x)
